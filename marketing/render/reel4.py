...
BEATS = [
    ("IMG_1725", 0.30, 2.60,  1),   # open water, deliberately unplaceable
    ("IMG_1705", 3.55, 2.20, -1),   # city lights on the water at dusk
    (None,       0.00, 3.90,  0),   # the app: the surprise card
    ("IMG_1727", 0.35, 2.20,  1),   # skyline from the ferry
    ("IMG_1728", 0.45, 2.00, -1),   # two people walking away with cases
    ("IMG_9402", 1.30, 3.40,  1),   # sunset over the water, end card
]
...
for bi, (clip, tin, dur, _) in enumerate(BEATS):
    ...
    n = counts[bi] + XFN + 2
    subprocess.run([
        "ffmpeg", "-v", "error", "-ss", str(max(0, tin - XF)), "-i", str(CLIPS / f"{clip}.mp4"),
        "-frames:v", str(n), "-vf",
...
window.setT=function(t){{
  ...
  const o1=plate('p1',t,B0-0.35,B1-0.10,0.40);
  const o2=plate('p2',t,B1+0.28,APP_IN-0.08,0.42);
  const o3=plate('p3',t,APP_IN+0.20,APP_OUT-0.05,0.34);
  const o4=plate('p4',t,B3+0.32,B4-0.10,0.42);
  const o5=plate('p5',t,B4+0.30,B5+0.05,0.42);

  E('scrim').style.opacity=Math.max(o1,o2,o4*0.95,o5*0.95)*0.94;
