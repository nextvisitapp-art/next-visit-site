...
BEATS = [
    ("IMG_1709",     0.40, 2.10,  1),   # the suite, walking in
    ("IMG_1709",    14.20, 1.70, -1),   # the curved window, the city
    (None,           0.00, 2.10,  0),   # app: the history, every trip listed
    (None,           0.00, 2.60,  0),   # app: inside one, the photos and the song
    ("IMG_1730",     1.20, 3.40,  1),   # the river: instruction and end card, no cut
]
# screen recordings for the app beats, keyed by beat index
APP_SRC = {
    2: ("ScreenRecording_07-19-2026 11-48-19_1", 1.00),   # the list of past trips
    3: ("ScreenRecording_07-19-2026 11-48-19_1", 5.40),   # scrolling one memory to the song
}
...
<div class="plate" id="p3" style="top:250px"><div class="eyebrow">Our story so far</div>
  <h1 class="sm">Unless you<br><em>kept</em> it.</h1></div>
<div class="plate" id="p4" style="top:250px"><div class="eyebrow">Every one of them has</div>
  <h1 class="sm">The photos. And<br>the <em>song.</em></h1></div>
<div class="plate" id="p6"><h1 class="sm">Start it <em>before</em><br>you go.</h1></div>
...
const B=[{B[0]:.4f},{B[1]:.4f},{B[2]:.4f},{B[3]:.4f},{B[4]:.4f}];
const APP_A={APP_A:.4f}, APP_B={APP_B:.4f}, APP_END={APP_END:.4f};
...
window.setT=function(t){{
  ...
  // one navy block and one phone across all three app beats. The recording
  // inside changes; the phone never leaves.
  const app=band(t,APP_A,APP_END,0.32);
  E('navy').style.opacity=app;
  E('ph').style.opacity=app;
  E('ph').style.transform='translateX(-50%) scale('+(1+0.030*cl(APP_A,APP_END,t)).toFixed(4)+')';

  const o1=plate('p1',t,B[0]-0.35,B[1]-0.24,0.34);
  const o2=plate('p2',t,B[1]-0.14,APP_A+0.26,0.30);
  const o3=plate('p3',t,APP_A+0.20,APP_B-0.04,0.26);
  const o4=plate('p4',t,APP_B+0.16,APP_END+0.20,0.26);
  const o6=plate('p6',t,B[4]+0.25,B[4]+2.15,0.30);

  E('scrim').style.opacity=Math.max(o1,o2,o6*0.95,o3*0.5,o4*0.5)*0.94;

  const oe=ss(cl(B[4]+2.28,B[4]+3.10,t));
  E('end').style.opacity=oe;
  E('scrimb').style.opacity=Math.max(oe*0.92,o6*0.30);
}};
