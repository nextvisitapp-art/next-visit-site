...
# NOTE: callers must increment STEP before calling — a $(subshell) call
# cannot mutate the parent shell's counter, which previously made every
# stage reuse step_1 and collide with its own input.
next_intermediate() {
...
if [[ "$DO_TRIM" == true ]]; then
    echo "--- Step: Trim ---"
    STEP=$((STEP + 1)); NEXT="$(next_intermediate)"
