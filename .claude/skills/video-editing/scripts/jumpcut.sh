...
# Segments are re-encoded (not stream-copied): -c copy can only cut on
# keyframes, so on long-GOP sources the cuts land seconds away from the
# silence boundaries and produce broken segments. veryfast/crf18 keeps
# this visually lossless and frame-accurate.
