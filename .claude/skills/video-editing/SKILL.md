---
name: video-editing
description: Edit video files from natural language - trim/cut, auto jump cuts (silence removal), burned-in captions (Hormozi/standard/minimal styles), text overlays, and speed changes via ffmpeg and Whisper. Use when asked to edit, cut, trim, caption, subtitle, speed up, or clean up a video file, or to turn raw footage or screen captures into reels or clips.
---
...
Cloud/web Claude Code sessions: fresh containers do not ship ffmpeg — run
`apt-get update && apt-get install -y ffmpeg` first. Whisper is optional and
only needed to transcribe; captioning from an existing SRT works without it.
